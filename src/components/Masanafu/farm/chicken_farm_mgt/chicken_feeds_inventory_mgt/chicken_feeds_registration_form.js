...
const RegisterChickenFeeds = () => {
    const [ feedsName, setFeedsName ] = useState('')
    ...
    const itemInputHandler = event => {
        ...
        setFeedsName(event.target.value)
    }
    ...
    const registerFeedsHandler = async (event) => {
        event.preventDefault()
        let res = await axios.post('http://82.180.136.230:3005/registerchickenfeeds', {
            token: localStorage.getItem('token'),
            productName: feedsName,
            unitPrice: unitPrice
        })
                    ...
                    <Form style={{ justifyContent: 'center', marginTop: '20px' }}>
                        <div className="mb-3">
                            <div className="form-floating mb-3">
                                ...
                                <label for="floatingInput">Feeds Name</label>
                            </div>
                            <div className="form-floating mb-3">
                                ...
                                <label for="floatingInput">Unit Price per KG ( UGX ) </label>
                            </div>
                            <span><button className="btn btn-outline-primary" onClick={registerFeedsHandler}>Add Item</button></span>
                        </div>
                    </Form>
                </div>
            </Col>
        ...
        <Row>
            <Col sm='12' md='8' lg='8' xl='8'>
                    <table className="table table-dark">
                        ...
                        <tbody>
                            {isFetchedFeedsLoading ? <tr><td>Loading Data From Database</td></tr> :
                                fetchedFeeds.map(item => (
...
export default RegisterChickenFeeds