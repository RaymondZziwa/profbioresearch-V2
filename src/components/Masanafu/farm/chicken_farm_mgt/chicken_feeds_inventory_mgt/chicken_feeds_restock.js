...
const ChickenFeedsRestockingForm = () => {
    ...
    const saveRestockData = async event => {
        event.preventDefault()
         let res = await axios.post('http://82.180.136.230:3005/savechickenfeedsrestockdata',{
            token:localStorage.getItem('token'),
...
export default ChickenFeedsRestockingForm