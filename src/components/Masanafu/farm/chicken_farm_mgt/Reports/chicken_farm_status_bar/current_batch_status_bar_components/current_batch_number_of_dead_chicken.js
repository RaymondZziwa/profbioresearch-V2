...
const CurrentBatchNumberOfDeadChicken = ({ batchData }) => {
    ...
    const [isLoading, setIsLoading] = useState(true)
      ...
      useEffect(() => {
        if (batchData.length > 0) {
            ...
            const getNumberOfDeadChickensForActiveBatch = (batchData) => {
                ...
                return activeBatch ? activeBatch.chickendead : 0;
            }
      ...
      return(
        <span style={{ borderRadius:'20px', textAlign:'center', width:'300px', backgroundColor:'white', boxShadow:'5px 5px #888888'}}>
            <h3 style={{padding:'10px', color:'black'}}>Total Number Of Dead Chicken In Current Batch</h3>
            <p style={{fontSize:'30px',color:'#29AB87'}}>{(!isLoading && total > 0) && <p> {total} </p>}</p>
        </span>
    )
}

export default CurrentBatchNumberOfDeadChicken